...
async def test_all_api_endpoints():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(f"{base_url}/api/chatbot/explain", json=payload) as response:
                data = await response.json()
                print(f"   Status: {response.status}")
                print(f"   Response: {json.dumps(data, indent=2)[:200]}...")
                assert response.status == 200, f"Expected 200, got {response.status}"
    ...
    # Test 9: Get Document by ID (from semantic search)
    ...
    print("All endpoints are accessible and returning expected responses.")
    print("Note: Some endpoints may return empty results if vector database is empty.")
